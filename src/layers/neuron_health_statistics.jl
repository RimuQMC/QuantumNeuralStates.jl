...
function _print_stats(name, arr; io=stdout)
    v = vec(arr)
    # Force scalar reduction results to host (safe: this is a diagnostic-only sync point)
    m   = Float32(mean(v))
    s2  = Float32(var(v))
...
function jacobian_statistics(ansatz, J::AbstractMatrix; io=stdout, lambda::Float32=1f-3, idx::Int=0)
