"""
    apply_loss_mode!(tmp, E_locs, w, E_mean, variance, raw_norm, α, mode)
...
```
"""
function apply_loss_mode!(tmp, E_locs, w, E_mean, variance, raw_norm, α, mode)
    if mode === :energy
        @. tmp += α * 2 * w * (E_locs - E_mean)
    elseif mode === :variance
        @. tmp += α * 2 * w * ((E_locs - E_mean)^2 - variance)
    elseif mode === :std
        σ = sqrt(variance)
        @. tmp += α * w * ((E_locs - E_mean)^2 - variance) / σ
    elseif mode === :logvar
        @. tmp += α * 2 * w * ((E_locs - E_mean)^2 - variance) / (variance + 1)
    elseif mode === :norm
        @. tmp += α * 4 * w * raw_norm * (raw_norm - 1)
    else
...
"""
    apply_loss_composite!(tmp, E_locs, w, E_mean, variance, raw_norm, modes)
...
"""
function apply_loss_composite!(tmp, E_locs, w, E_mean, variance, raw_norm, modes)
    for (mode, α) in modes
        apply_loss_mode!(tmp, E_locs, w, E_mean, variance, raw_norm, α, mode)
    end
end
...
This function calculates, in-place `tmp`, gradients of loss function w.r.t. logψ. It 
handle single loss function but also composite loss functions defined in `mode`.

# Variables

* `tmp`: array buffer (vector) in which gradients are saved.
...
"""
function apply_loss!(tmp, E_locs, w, E_mean, variance, raw_norm, mode)
    fill!(tmp, 0.0)

    if mode isa Tuple
        apply_loss_composite!(tmp, E_locs, w, E_mean, variance, raw_norm, mode)
    else
        apply_loss_mode!(tmp, E_locs, w, E_mean, variance, raw_norm, 1.0, mode)
    end
end
